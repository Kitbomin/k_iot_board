...
public class FileServiceImpl {
    ...
    private void ensureDirectory(String path) {
        ...
        if(!dir.exists()) {
            // 업로드하려는 경로가 실제 디렉토리에 존재하지 않으면? => mkdir로 만들겠다
            // mkdirs(): 존재하지 않는 상위 디렉토리까지 모두 생성함
            dir.mkdirs();
        }
    }
    ...
    public FileInfo saveUserProfile(MultipartFile file) {
        ...
        try {
            ...
            // Paths.get() 으로 경로값(문자열)을 받아 Paht 객체를 생성함
            Path path = Paths.get(fullDir + "/" + storedName);
            // InputStream 으로 데이터를 읽어 path 위치에 파일을 생성 (또는 덮어쓰기)함
            //  +) REPLACE_EXISTING: 덮어쓰기 옵션임
