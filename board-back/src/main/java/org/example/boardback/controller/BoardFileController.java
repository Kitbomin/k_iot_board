...
public class BoardFileController {
    ...
    public ResponseEntity<?> uploadBoardFiles (
            @PathVariable Long boardId, @RequestParam("files")List<MultipartFile> files
            ) {
    ...
    public ResponseEntity<?> updateBoardFiles(
            ...
            // ModelAttribute: 쿼리 파라미터나 HTML 폼 데이터를 Java 객체로 바인딩할 때 사용
            // MultipartFile + List<Long> 조합 시 JSON + 파일 혼합이 불가능함
            //  >> FormData 기반 요청에는 @RequestBody 사용이 불가능함
            //  >> 그래서 나온게 @ModelAttribute 가 가장 안정적인 방식임
