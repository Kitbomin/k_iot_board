...
type BoardEditorProps = {
  mode?: Mode; boardId?: number; onSaved?: (boardId?: number) => void;
}
...
function BoardEditor({mode = "create", boardId, onSaved } : BoardEditorProps) {
  ...
  const toggleKeep = (fileId: number) => {
    setKeepIds(prev => (prev.includes(fileId) // 기존 목록에 체크박스 클릭한 요소가 있을 때
                        ? prev.filter(id => id !== fileId) // 기존 목록에서 삭제
                        : [...prev, fileId])) // 기존 목록에 추가
  }
  ...
  return (
    <Wrap>
      ...
      <Row>
        ...
        <input type="file" ref = {fileInputRef} multiple onChange={onFileChange}/>
